...
class HermesUiHandler(BaseHTTPRequestHandler):
    ...
    def _proxy_ttyd_http(self) -> None:
        ...
        try:
            with urllib.request.urlopen(request, timeout=30) as response:
                ...
                # Inject WebSocket URL patch into ttyd's HTML.
                if "text/html" in content_type:
                    inject = self._TTYD_WS_PATCH.encode()
    ...
    _PANEL_JS_PATCH = (
        ...
        "}"
        # ── EventSource (SSE for chat stream) ──────────────────────────────
        "var _ES=window.EventSource;"
        "if(_ES){"
        "var ESProxy=function(url,opts){"
        "return opts?new _ES(rewrite(url),opts):new _ES(rewrite(url));"
        "};"
        "ESProxy.prototype=_ES.prototype;"
        "['CONNECTING','OPEN','CLOSED'].forEach(function(k){ESProxy[k]=_ES[k];});"
